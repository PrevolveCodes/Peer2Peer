...
import {getDatabase,ref,get,update,set,remove} from 'https://www.gstatic.com/firebasejs/10.8.0/firebase-database.js';
...
async function openRoomSettings(code){
 ...
 try{
  ...
  document.getElementById('p2p-rs-save').onclick=async()=>{const btn=document.getElementById('p2p-rs-save');btn.disabled=true;try{
   let pic=room.avatarData||null;const f=document.getElementById('p2p-rs-pic').files[0];if(f)pic=await new Promise(res=>{const r=new FileReader();r.onload=()=>res(r.result);r.readAsDataURL(f)});
   const name=document.getElementById('p2p-rs-name').value.trim()||code;const permissions={};
   document.querySelectorAll('#modal-root [data-p2p-perm]').forEach(i=>{const u=i.dataset.uid,p=i.dataset.p2pPerm;permissions[u]??={};permissions[u][p]=!!i.checked});
   await update(ref(db,`rooms/${code}/meta`),{name,avatarData:pic});
   await set(ref(db,`rooms/${code}/meta/permissions`),permissions);
   const verify=await get(ref(db,`rooms/${code}/meta`));const saved=verify.val()||{};
   for(const i of document.querySelectorAll('#modal-root [data-p2p-perm]')){const actual=!!saved.permissions?.[i.dataset.uid]?.[i.dataset.p2pPerm];if(actual!==!!i.checked)throw Error('Firebase rejected the permission change. Check your Realtime Database rules for rooms/*/meta.');}
   close();const row=document.querySelector(`[data-room="${CSS.escape(code)}"]`);row?.querySelector('span')?.replaceChildren(document.createTextNode(name));
  }catch(e){console.error(e);alert(`Could not save room settings: ${e?.message||e}`)}finally{if(document.getElementById('p2p-rs-save'))document.getElementById('p2p-rs-save').disabled=false}};
