...
function addRemoveButtons(){
  document.querySelectorAll('#dm-list [data-dm]').forEach(dm=>{
    if(dm.closest('.friend-row'))return;
    const uid=dm.dataset.dm;
    if(!uid||!me)return;
    const wrap=document.createElement('div');
    wrap.className='friend-row';
    wrap.style.cssText='display:flex;align-items:center;width:100%;gap:5px';
    ...
    removeBtn.style.cssText='flex:0 0 auto;cursor:pointer';
...
function addDeleteButtons(){
  document.querySelectorAll('#messages .message[data-message-id]').forEach(msg=>{
    ...
    if(!uid||!id||!actions||!me||uid!==me.uid)return;

    const b=document.createElement('button');
    b.type='button';
    b.dataset.fixDelete='1';
    b.title='Delete message';
    b.textContent='🗑';
    ...
    b.onclick=async e=>{
      ...
      try{
        if(activeType==='dm'&&activeDM){
          ...
          throw new Error('Open the conversation again and try deleting the message.');
        }
