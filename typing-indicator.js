...
async function resolvePath(){const title=$('view-title')?.textContent?.trim();const sub=$('view-sub')?.textContent?.trim();if(!title||title==='Welcome'||!me)return null;if(sub==='Private conversation'){
  const snap=await get(ref(db,'userDirectory'));const users=snap.val()||{};const other=Object.values(users).find(u=>u.uid!==me.uid&&(u.username||'')===title);if(other)return `dms/${[me.uid,other.uid].sort().join('_')}/messages`;
}
if(sub==='Room'){
  const joined=await get(ref(db,`users/${me.uid}/joinedRooms`));const codes=Object.keys(joined.val()||{});for(const code of codes){const snap=await get(ref(db,`rooms/${code}/meta`));const room=snap.val()||{};if((room.name||code)===title)return `rooms/${code}/messages`}
}
return null}
async function attach(){const composer=$('msg');if(!composer)return;if(composer.dataset.typingReady)return;composer.dataset.typingReady='1';
  const path=await resolvePath();if(!path)return;currentPath=path;
  const indicator=$('typing-indicator');if(indicator&&typingListener)typingListener();
  typingListener=onValue(ref(db,typingBase(path)),s=>renderTyping(s.val()||{}));
  composer.addEventListener('input',()=>{const hasText=composer.value.trim().length>0;if(hasText)setTyping(path);else stopTyping()});
  composer.addEventListener('blur',stopTyping);
}
